...
bool myObject3D::readMesh(string filename)
{
	...
	if (!fin.is_open())
	{
		cout << "ERROR at (" << __FILE__ << ", " << __LINE__ << ", " << __func__ << "): " << "Unable to open file.\n";
		return false;
	}
	while (getline(fin, s))
	{
		...
		{
			while (myline >> index1>>index2>>index3)
			{
...
void myObject3D::computeNormals()
{
	...
	for (int i = 0; i < indices.size(); i++){
		glm::vec3 face_normal = glm::normalize(glm::cross(vertices[indices[i][1]] - vertices[indices[i][0]], vertices[indices[i][2]] - vertices[indices[i][1]]));
